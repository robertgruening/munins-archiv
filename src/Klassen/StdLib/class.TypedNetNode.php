...
include_once(__DIR__."/class.NetNode.php");
include_once(__DIR__."/class.ListType.php");

class TypedNetNode extends NetNode
{
	// variables
	protected $_typ = NULL;
	...
	protected function GetSQLStatementToLoadRootsByTypId($typId)
	{
		return "SELECT t.Id
				FROM ".$this->GetTableName()." AS t LEFT JOIN ".
					...
					ON t.Id = tt.Id
				WHERE tt.Parent_Id IS NULL AND
					Typ_Id = ".$typId."
				ORDER BY t.Typ_Id ASC, t.Bezeichnung ASC;";
	}
	...
	protected function GetSQLStatementToLoadRoots()
	{
		return "SELECT t.Id
				...
				ORDER BY Typ_Id ASC, t.Bezeichnung ASC;";
	}	
}
?>

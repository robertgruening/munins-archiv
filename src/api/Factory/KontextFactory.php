...
class KontextFactory extends Factory implements iTreeFactory
{
    ...
    */
    ...
    #region load
	/**
	* Returns the SQL SELECT statement to load Id, Bezeichnung, Typ_Id, GeoPointLatitude (for Fundstelle), GeoPointLongitude (for Fundstelle), Datum (for Begehung) and Kommentar (for Begehung).
	*/
	protected function getSQLStatementToLoad()
	{
		return "SELECT ".$this->getTableName().".Id AS Id, ".$this->getTableName().".Bezeichnung AS Bezeichnung, ".$this->getTableName().".Typ_Id AS Typ_Id, ST_X(Fundstelle.GeoPoint) AS GeoPointLatitude, ST_Y(Fundstelle.GeoPoint) AS GeoPointLongitude, Begehung.Datum AS Datum, Begehung.Kommentar AS Kommentar 
			FROM ".$this->getTableName()." LEFT JOIN Fundstelle ON ".$this->getTableName().".Id = Fundstelle.Id LEFT JOIN Begehung ON ".$this->getTableName()."Id = Begehung.Id";
	}

	/**
	* Returns the SQL statement search conditions as string by the given search conditions.
	* Search condition keys are: Id, Bezeichnung, Typ_Id, HasFunde, HasParent, Parent_Id, HasChildren and Child_Id.
	*
